...
export async function updateConfiguration(configState: TConfigurationState) {
  console.log("state to save", configState);
  const currentTime = new Date()
    .toISOString()
    .substring(0, 19)
    .replace("T", " ");
  ...
  const objectToInsert = {
    ...
    panel_service_distribution:
      configState.Configuration.SelectedServiceDistribution,
  ...
  if (configState.Metadata.DatabaseID) {
    try {
      ...
      // Overall error handling
      console.error(
        "Update configurations error:",
        error,
        error.message,
        error.details
      );
      throw new Error(
        `Failed to update record in the database,${error.message}`
      );
    }
  } else {
    throw new Error(
      `Failed to update record in the database, No Database ID was provided`
    );
